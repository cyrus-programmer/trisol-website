...
function StackCard({ group, index }) {
  ...
  return (
    <FadeIn delay={index * 0.07}>
      ...
      >
        <div
          style={{
            ...
            zIndex: 1,
            background:
              "radial-gradient(280px circle at var(--x, 50%) var(--y, 50%), rgba(253,189,16,0.06), transparent 70%)",
          }}
        />
        <div
          style={{
      ...
      <style>{`.stack-card:hover { border-color: rgba(253,189,16,0.25) !important; transform: translateY(-5px); box-shadow: 0 20px 50px rgba(0,0,0,0.45), 0 0 30px rgba(253,189,16,0.05); }`}</style>
    </FadeIn>
  );
}
  ...
  return (
    <section
      ...
      style={{
        ...
        background:
          "linear-gradient(180deg, transparent, rgba(40,42,114,0.05), transparent)",
      }}
    >
      <div style={{ maxWidth: 1300, margin: "0 auto" }}>
        <FadeIn>
          <div style={{ marginBottom: "4rem" }}>
            ...
            >
              We choose tools for outcomes, not trends. These are the
              technologies that power every product we ship.
            </p>
          </div>
        </FadeIn>
        <div
        ...
        >
          {stacks.map((g, i) => (
            <StackCard key={g.category} group={g} index={i} />
          ))}
        </div>
      </div>
      <style>{`
        @media (max-width: 900px) { #stack > div > div:last-child { grid-template-columns: 1fr 1fr !important; } }
        @media (max-width: 600px) { #stack > div > div:last-child { grid-template-columns: 1fr !important; } }
      `}</style>
    </section>
  );
}

  ...
  return (
    <footer style={{
      ...
      <div style={{ display: 'flex', gap: '2rem' }}>
        {links.map(l => (
          l.isHash && onHome ? (
            <a key={l.label} href={l.href} style={{ fontSize: '0.8rem', color: 'rgba(238,234,248,0.4)', textDecoration: 'none' }}>{l.label}</a>
          ) : l.isHash ? (
            <a key={l.label} href={`/${l.href}`} style={{ fontSize: '0.8rem', color: 'rgba(238,234,248,0.4)', textDecoration: 'none' }}>{l.label}</a>
          ) : (
            <Link key={l.label} to={l.href} style={{ fontSize: '0.8rem', color: 'rgba(238,234,248,0.4)', textDecoration: 'none' }}>{l.label}</Link>
          )
        ))}
      </div>

      <span style={{ fontSize: '0.75rem', color: 'rgba(238,234,248,0.25)' }}>© 2025 Trisol Technologies</span>
